...
public class Bank
{
	private final Hashtable<String, Account> _accountlist = new Hashtable<>();
	...
	public void openAccount(String accountid) throws AccountExistsException
	{
		...
		else
		{
			...
			//adding account instead of getting it
			_accountlist.put(accountid, new Account(accountid, _currency));
		}
	}
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException
	{
		...
		{
			Account account = _accountlist.get(accountid);
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException
	{
		...
		else
		{
			Account account = _accountlist.get(accountid);
//			account.deposit(money);
//			withdrawing money instead of depositing ip
	...
	public Integer getBalance(String accountid) throws AccountDoesNotExistException
	{
		...
		{
			return _accountlist.get(accountid).getBalance().getAmount();
		}
	}
	...
	{
		if (!_accountlist.containsKey(fromaccount) || !tobank._accountlist.containsKey(toaccount))
		{
			throw new AccountDoesNotExistException();
		}
		else
		{
			_accountlist.get(fromaccount).withdraw(amount);
			tobank._accountlist.get(toaccount).deposit(amount);
		}
	}
...
//		transferring money to the proper account
	...
	{
		for (Account account : _accountlist.values())
		{
			account.tick();
		}
	}
